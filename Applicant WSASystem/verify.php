...
if (isset($_GET['email']) && isset($_GET['token'])) {
    ...
    $query = mysqli_prepare($conn, "SELECT * FROM `tbl_user` WHERE email = ? AND verification_token = ?");
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $update = mysqli_prepare($conn, "UPDATE `tbl_user` SET acc_status = 'verified' WHERE email = ?");
